...
class ExtendedTrimesh(trimesh.Trimesh):
    ...
    def calculate_contact_area(self, other_mesh, distance):
        ...
        return contact_area, contact_faces_indices
...
#     def get_potential_contact_faces(self, other_mesh, distance):
#         potential_contact_faces = []
#         contact_face_indices = set()  # to keep track of faces on the cell that are in contact
#         for face_index in range(len(other_mesh.faces)):
#             face = other_mesh.vertices[other_mesh.faces[face_index]]
#             centroid = np.mean(face, axis=0).reshape(1, -1)
#             dist, idx = self.my_kdtree.query(centroid)
#             if dist < distance:
#                 potential_contact_faces.append(face_index)
#                 # store the face index of the cell in contact
#                 contact_face_indices.add(idx[0])
                
#         with napari.gui_qt():
#             viewer = napari.Viewer()
#             viewer.add_surface((self.vertices, self.faces), name='target')
#             viewer.add_surface((self.vertices, self.faces[potential_contact_faces]), colormap='red', name='contact_faces')
#             viewer.add_surface((other_mesh.vertices, other_mesh.faces), name='neighbour')
#         return potential_contact_faces, contact_face_indices


#     def calculate_contact_area(self, other_mesh, distance):
#         contact_faces_indices, cell_contact_face_indices = self.get_potential_contact_faces(other_mesh, distance)
#         # Assuming contact area is just the sum of the areas of the contact faces
#         contact_area = np.sum(self.area_faces[list(cell_contact_face_indices)])  # sum the areas of the contact faces on the cell
#         return contact_area
...
# def create_vtk_file(mesh_dir, statistics_csv, statistic_column, output_vtk, normalize=False):
#     """
...
def create_vtk_file(mesh_dir, statistics_csv, output_vtk, normalize=False):
    """
    Create a VTK file with 3D cell meshes and colors based on all numeric columns in the CSV file.
    ...
    normalize: bool
        Whether to normalize the statistic values to the range [0, 1]. Default is False.
    """
    ...
    # Create an empty MultiBlock dataset
    all_meshes = pv.MultiBlock()

    for _, row in statistics_df.iterrows():
        mesh_file = os.path.join(mesh_dir, f'cell_{int(row["cell_id"])-1}.stl')
        if os.path.exists(mesh_file):
            ...
            for column in numeric_columns:
                value = row[column]
                if normalize:
                    value = (value - statistics_df[column].min()) / (statistics_df[column].max() - statistics_df[column].min())
                mesh.point_data[column] = np.full(mesh.n_points, value)
            all_meshes.append(mesh)

    # Write the MultiBlock dataset to a VTK file
    all_meshes.save(output_vtk)
