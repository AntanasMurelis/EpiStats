...
"""
    ...
    UPDATE on 2023.04.20: added function relabel_image and removed padding from vertices coordinates
"""

# todo: put in the correct absolute path
SEGMENTED_PATH = '/Volumes/maoxia-1/relabeled/cell_boundary_time_40_relabeled.tif'
OUTPUT_PATH = '/Volumes/maoxia-1/cell_stats/cell_boundary_time_40_stats.csv'
MESH_PATH = '/Volumes/maoxia-1/cell_meshes/cell_boundary_time_40'


def relabel_image(image):
    """
    Relabel the segmented image to make the labels consecutive, starting from 1, and 0 for the background
    :param image: np.array
    :return: relabeled_image: np.array
    """
    if np.all(np.diff(np.unique(image)) == 1):
        # if the labels are already consecutive
        return image
    else:
        # Get the unique labels in the image and the label mapping that maps each
        # pixel value in the image to its corresponding index in the unique labels array.
        # The label mapping is a 1D integer array with the same shape as the input image.
        # The label mapping is used to convert the input image to an array of indices
        # into the unique labels array.
        unique_labels, label_mapping = np.unique(image, return_inverse=True)

        # Find the label with the highest frequency in the label mapping array. This
        # corresponds to the label that is most likely to be the background label.
        background_label = np.argmax(np.bincount(label_mapping))

        # Modify the label mapping array to relabel the background label to 0.
        label_mapping[label_mapping == background_label] = 0

        # Convert the label mapping array back to an image with the same shape as the input image,
        # with the background label set to 0.
        relabeled_image = label_mapping.reshape(image.shape)
...
def convert_cell_labels_to_meshes(
    ...
    for label_id in np.unique(img):
        ...
        points = points - pad_width  # remove padding from vertices coordinates
...
def collect_cell_morphological_statistics(labeled_img, img_resolution, contact_cutoff):
    ...
    mesh_lst = convert_cell_labels_to_meshes(labeled_img, img_resolution, smoothing_iterations=4)

    # Create a folder if doesn't already exists to store the meshes of the cells
    if not path.exists(MESH_PATH): mkdir(MESH_PATH)

    # Save the meshes of the cells to this folder
    for cell_id, mesh in enumerate(mesh_lst): mesh.export(path.join(MESH_PATH, f"cell_{cell_id}.stl"))
    ...
    cell_contact_area_fraction_lst = list(np.full(len(cell_elongation_lst), np.nan))
    ...
    cell_principal_axis_lst = [(" ").join(map(to_scientific_str, principal_axis.tolist())) for principal_axis in
                               cell_principal_axis_lst]
...
def main():
    ...
    if test:
        ...
        imwrite('test_segmentation.tif', img)
    else:
        segmented = imread(SEGMENTED_PATH)
        # make the labels consecutive
        relabeled_img = relabel_image(segmented)
        # make the pixel dimension order [x, y, z]
        img = np.einsum('kij->ijk', relabeled_img)

    # Collect the statistics of the cells
    cell_statistics_df = collect_cell_morphological_statistics(img, np.array([0.236, 0.236, 0.487]), 1)
    # cell_statistics_df = collect_cell_morphological_statistics(img, np.array([1,1,2]), 1)

    # Save the cell statistics to a csv file
    cell_statistics_df.to_csv(OUTPUT_PATH, sep=",", index=False)

    print(f'stats collection finished successfully, costing {timer() - start} s. \n')
