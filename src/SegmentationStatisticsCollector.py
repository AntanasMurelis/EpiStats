...
def reconstruct_contact_matrix(cell_id_lst: list, cell_neighbors_lst: list, cell_contact_area_distribution_dict: dict):
    """
    Reconstruct the contact matrix given the neighbors and the distribution of contact areas.
    ...
    -----------
        ...
        cell_neighbors_lst (list):
            List of lists where the i-th list contains the IDs of the neighbors of the i-th cell.

        cell_contact_area_distribution_dict (dict):
            Dictionary where each key is a cell ID and each value is a list of contact areas corresponding to the neighbors of the cell in the same order as in cell_neighbors_lst.

    Returns:
    --------
        contact_matrix (np.ndarray):
            A matrix where each element represents the contact area between two cells. The matrix is not necessarily symmetric.
    """
    num_cells = len(cell_id_lst)
    contact_matrix = np.zeros((num_cells, num_cells))

    for cell_id, neighbors in zip(cell_id_lst, cell_neighbors_lst):
        contact_areas = cell_contact_area_distribution_dict[cell_id]
        for neighbor, contact_area in zip(neighbors, contact_areas):
            # Subtract 1 from cell_id and neighbor because cell IDs start from 1 but Python's list indices start from 0
...
def evaluate_matrix_symmetry(A: np.ndarray) -> float:
    ...
    Evaluate the symmetry of a matrix using the formula:
        symmetry = 1 - ((1/2) * (norm(A - A.T) / norm(A)) ^ 2)
    ...
    --------
    float
        The symmetry of the matrix. 1 means perfectly symmetric, 0 means not symmetric at all.
    """

    norm_diff = np.linalg.norm(A - A.T, ord='fro')
...
def calculate_all_statistics(mesh_lst: list, cell_id_lst: list, cell_neighbors_lst: list, filtered_cell_id_lst: list, 
    ...
    # create dictionary with cell ID as keys and contact area distribution as values
    cell_contact_area_distribution_dict = {cell_id: value[1] for cell_id, value in cell_contact_area_fraction_dict.items()}

    # reconstruct the contact matrix
    contact_matrix = reconstruct_contact_matrix(cell_id_lst, cell_neighbors_lst, cell_contact_area_distribution_dict)
 
    print(f'Symmetry of the neighbour area matrix {evaluate_matrix_symmetry(contact_matrix)}')
...
if __name__ == "__main__":
    ...
    # for i in (5, 10):
    #     img = f"/Users/antanas/Downloads/relabeled/cell_boundary_time_{i}_relabeled.tif"
    #     cell_statistics_df = collect_cell_morphological_statistics(img, np.array([0.236, 0.236, 0.487]), contact_cutoff = 0.5, output_folder=f"/Users/antanas/BC_Project/Control_Segmentation/Maoxia_{i}", meshes_only=False, overwrite=False,
    #                                                            smoothing_iterations=4, erosion_iterations=2, dilation_iterations=3, calculate_contact_area_fraction=True, plot = 'all', plot_type = 'violin', preprocess = True, max_workers=None, volume_threshold=i)
